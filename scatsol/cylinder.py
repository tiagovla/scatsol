    ...
    *,
    ...
    if cylinder == None:  # if PEC cylinder
        if pol == "TM":
            ...
            Erpz[mask], Hrpz[mask] = _calculate_field(
                xyz[mask], bg.k, bg.eta, an, hankel2, h2vp
            )
        else:
            an = _an_cond_te(bg.k, radius, n)
            Hrpz[mask], Erpz[mask] = _calculate_field(
                xyz[mask], bg.k, 1 / bg.eta, an, hankel2, h2vp
            )
            Erpz[mask] = -Erpz[mask]
        ...
        if pol == "TM":
            ...
            Hrpz[~mask], Erpz[~mask] = _calculate_field(
                xyz[~mask], c.k, 1 / c.eta, cn, jv, jvp
            )
            Erpz[mask] = -Erpz[mask]

    Ei, Hi = mie_incident_field(
        xyz[mask], radius, frequency, background, pol, n=n
    )
    Erpz[mask] += Ei
    Hrpz[mask] += Hi
...
def _an_cn_diel_tm(
    ...
    nn = np.arange(0, n)
    den = eta * hankel2(nn, k * a) * jvp(nn, kd * a) - etad * h2vp(
        nn, k * a
    ) * jv(nn, kd * a)
    num = eta * jv(nn, k * a) * jvp(nn, kd * a) - etad * jvp(nn, k * a) * jv(
        nn, kd * a
    )
    an = -(1j ** (-nn)) * num / den
...
def _calculate_field(
    ...
    h_field[:, 0] = (nn * an * eps @ (besselterm * sinterm)) / (
        1j * k * eta * rho
    )
    h_field[:, 1] = (k * an * eps @ (besselpterm * costerm)) / (
        1j * k * eta * rho
    )
